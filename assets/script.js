...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Check if this is an external link or a link to another page
            if (targetHref.startsWith('http') || targetHref.includes('.html')) {
                // Let the default behavior handle external links and page navigation
                return;
            }
            ...
            const targetId = targetHref.includes('#') ? targetHref : `#${targetHref}`;
            const targetSection = document.querySelector(targetId);
    ...
    function setActiveNavOnScroll() {
        ...
        sections.forEach((section, index) => {
            ...
            if (scrollPosition >= sectionTop && scrollPosition < sectionBottom) {
                ...
                // Ajouter la classe active au lien correspondant
                // Modifier cette partie pour prendre en compte différentes structures d'URL
                const possibleSelectors = [
                    `.menu a[href="#${id}"]`,
                    `.menu a[href="${window.location.pathname}#${id}"]`,
                    `.menu a[href="${window.location.pathname.replace(/\.html$/, '')}#${id}"]`
                ];

                // Chercher le premier sélecteur qui correspond
                let activeLink = null;
    ...
    function ensureLastSectionHeight() {
        ...
        if (lastSection && lastSection.id !== "Work" && lastSection.offsetHeight < viewportHeight) {
            lastSection.style.minHeight = viewportHeight + 'px';
        }
    }
    ...
    if (window.location.hash) {
        ...
        setTimeout(function() {
            ...
            if (targetSection) {
                ...
                // Essayer différentes structures d'URL pour trouver le lien actif
                const possibleSelectors = [
                    `.menu a[href="${targetId}"]`,
                    `.menu a[href="${window.location.pathname}${targetId}"]`,
                    `.menu a[href="${window.location.pathname.replace(/\.html$/, '')}${targetId}"]`
                ];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Déterminer si nous sommes sur une page de CV (resume) ou d'index
    const isResumePage = window.location.pathname.includes('resume');

    // Déterminer la langue actuelle
    const isCurrentlyFrench =
        window.location.pathname.includes('_fr') ||
        window.location.pathname.includes('_FR') ||
        langSwitch.checked;
    ...
    langSwitch.addEventListener('change', function() {
        ...
        // Construire la nouvelle URL basée sur le type de page actuel
        let newPath;
        const currentPath = window.location.pathname;

        if (isResumePage) {
            // Pour les pages CV
            newPath = newLanguage === 'fr' ? 'resume_fr.html' : 'resume_en.html';
        } else {
            // Pour les pages d'index
            newPath = newLanguage === 'fr' ? 'index_fr.html' : 'index_en.html';
        }

        // Préserver le hash si présent
        if (window.location.hash) {
